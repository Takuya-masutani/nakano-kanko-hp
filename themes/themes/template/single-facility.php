...
<article id="pageid-post<?php the_ID(); ?>" class="entry-content inner">
	<div class="facility-content flex">
		<!-- メインスライダー -->
		<?php
		$images = [];
		for ($i = 1; $i <= 5; $i++) {
			$field = get_field("facility-images0{$i}");
			if ($field) $images[] = $field;
		}
		?>
		<?php if (!empty($images)): ?>
		<div class="facility-slider">
			<div class="swiper facility-main">
			...
			<!-- スライダーサムネイル -->
			<div class="swiper facility-thumbs">
				<div class="swiper-wrapper">
					<?php foreach ($images as $img): ?>
					<div class="swiper-slide">
						<img src="<?php echo esc_url($img); ?>" alt="<?php the_title_attribute(); ?>">
		...
		<div class="facility-info">
			...
			<dl>
				<!-- 営業時間 -->
				<?php if ( $culture_hours = get_field('facility-hours') ) : ?>
				<dt class="hours">営業時間</dt>
				<dd class="hours"><?php echo esc_html($culture_hours); ?></dd>
				<?php endif; ?>
				...
				<!-- 住所 -->
				<?php if ( $culture_location = get_field('facility-location') ) : ?>
				<dt class="location">住所</dt>
				<dd class="location"><?php echo esc_html($culture_location); ?></dd>
				<?php endif; ?>

				<!-- Google Map -->
				<?php if ( $culture_map = get_field('facility-googlemap') ) : ?>
				<div class="googlemap">
					<?php echo $culture_map; // そのまま出力 ?>
				</div>
				<?php endif; ?>
	...
	<!-- コメント -->
	<?php if ( $culture_comment = get_field('facility-comment') ) : ?>
	<div class="comment">
		<p><?php echo wp_kses_post( nl2br( esc_html( $culture_comment ) ) ); ?></p>
	</div>
	<?php endif; ?>

	<!-- ウェブサイト -->
	<?php if ( $culture_website = get_field('facility-website') ) : ?>
	<dl class="facility-info w100">
		<dt class="website">ウェブサイト</dt>
		<dd class="website">
			<a class="externalLink" href="<?php echo esc_url($culture_website); ?>" target="_blank" rel="noopener">
